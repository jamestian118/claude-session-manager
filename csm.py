...
def cmd_graphiti_sync(args):
    """同步会话数据到 Graphiti MCP Server"""
    from lib.graphiti_sync import sync_session, sync_all_recent

    if getattr(args, "all", False):
        limit = getattr(args, "limit", 20)
        print(f"批量同步最近 {limit} 个会话到 Graphiti...")
    ...
    tool_str = getattr(args, "tool", None) or "claude"
    print(f"同步会话 {sid[:8]} ({tool_str}) 到 Graphiti...")
    result = sync_session(sid, tool_str)
    if "error" in result:
...
def main():
    ...
    p_gsync = sub.add_parser("graphiti-sync", aliases=["gs"], help="同步会话数据到 Graphiti MCP Server")
    p_gsync.add_argument("session_id", nargs="?", help="会话 ID（支持短 ID 前缀匹配）")
    p_gsync.add_argument("--all", action="store_true", help="批量同步最近会话")
    p_gsync.add_argument("--limit", type=int, default=20, help="批量同步数量（默认 20）")
    ...
    if args.command == "mcp":
        return cmd_mcp(args)
    if args.command in ("graphiti-sync", "gs"):
        return cmd_graphiti_sync(args)
