...
def _missing_snapshot_message(session: SessionSummary) -> str:
    ...
    return (
        ...
        "  /usr/bin/python3 $HOME/Library/Application Support/cli-handoff-bundle/bin/ai_handoff_watch.py once\n"
...
def open_mcp_handoff_in_terminal(session: SessionSummary, target: ToolType) -> tuple[bool, str]:
    ...
    prompt = build_mcp_takeover_prompt(session.session_id, target.value)
