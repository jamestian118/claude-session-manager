...
class CodexProvider(BaseProvider):
    ...
    def _build_sessions_cache_key(self) -> tuple:
        ...
        if db_path:
            db_key = (str(db_path),) + self._safe_stat_key(db_path)
        else:
            db_key = ("", 0, 0)

        session_files: list[tuple[str, int, int]] = []
        for d in (self._sessions_dir, self._archived_dir):
            ...
            for jsonl in jsonl_files:
                mtime_ns, size = self._safe_stat_key(jsonl)
                session_files.append((str(jsonl), mtime_ns, size))
        session_files.sort()
        return history_key, db_key, tuple(session_files)
    ...
    def delete_session(self, session_id: str) -> dict[str, bool]:
        ...
        if self._global_state_file.exists():
            try:
                with open(self._global_state_file, encoding="utf-8", errors="replace") as f:
                    state = json.load(f)
