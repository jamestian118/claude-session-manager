...
class ClaudeProvider(BaseProvider):
    def __init__(self) -> None:
        self._sessions_cache_key: tuple | None = None
    ...
    @staticmethod
    def _safe_stat_key(path: Path) -> tuple[int, int]:
        try:
            st = path.stat()
    ...
    def _build_sessions_cache_key(self) -> tuple:
        history_key = self._safe_stat_key(self._history_file)
        project_files: list[tuple[str, int, int]] = []
        if self._projects_dir.exists():
            try:
                project_dirs = list(self._projects_dir.iterdir())
            except OSError:
                project_dirs = []
            for project_dir in project_dirs:
                ...
                for jsonl_file in jsonl_files:
                    mtime_ns, size = self._safe_stat_key(jsonl_file)
                    project_files.append((str(jsonl_file), mtime_ns, size))
        project_files.sort()
        return history_key, tuple(project_files)
    ...
    def _decode_project_dir(dirname: str) -> str:
        """将编码的项目目录名还原为路径，如 -Users-Zhuanz-Documents-Code -> /Users/Zhuanz/Documents/Code"""
    ...
    def load_sessions(self) -> list[SessionSummary]:
        cache_key = self._build_sessions_cache_key()
        if self._sessions_cache_key == cache_key and self._sessions_cache is not None:
        ...
        if self._history_file.exists():
            with open(self._history_file, encoding="utf-8", errors="replace") as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
        ...
        self._sessions_cache_key = cache_key
    ...
    def delete_session(self, session_id: str) -> dict[str, bool]:
        ...
        if self._history_file.exists():
            ...
            if removed:
                self._atomic_write_lines(self._history_file, lines_to_keep)
                result["history"] = True
    ...
    def _rewrite_history_first_display(self, session_id: str, new_display: str) -> tuple[bool, str]:
        ...
        changed = False
        try:
            with open(self._history_file, encoding="utf-8", errors="replace") as src, open(tmp, "w", encoding="utf-8") as dst:
                for line in src:
                    stripped = line.strip()
                    if not stripped:
                        continue
    ...
    def load_stats(self) -> dict | None:
        if not self._stats_file.exists():
            return None
        try:
            with open(self._stats_file, encoding="utf-8", errors="replace") as f:
                return json.load(f)
