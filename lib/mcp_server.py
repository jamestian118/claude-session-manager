...
def _tool_error_message(result: object) -> str:
...
def _run_tool_with_logging(tool_name: str, fn):
    ...
    try:
        ...
        if error_message:
            logger.warning(
                "mcp request tool=%s duration_ms=%.2f status=error error=%s",
                tool_name,
                duration_ms,
                error_message,
            )
            ...
            logger.info(
                "mcp request tool=%s duration_ms=%.2f status=success",
                tool_name,
                duration_ms,
            )
        return result
...
def list_sessions(tool_filter: str = "", limit: int = 20) -> list[dict]:
    return _run_tool_with_logging(
        ...
        lambda: [
            _session_to_dict(s)
            for s in store.load_sessions(_tool_type_from_str(tool_filter) if tool_filter else None)[:limit]
        ],
    )
...
def search_sessions(query: str, limit: int = 10) -> list[dict]:
    return _run_tool_with_logging(
        ...
        lambda: [
            _session_to_dict(s)
            for s in store.search_sessions(store.load_sessions(), query)[:limit]
        ],
    )
...
def get_session_context(session_id: str, tool_type: str) -> dict:
    ...
    return _run_tool_with_logging("get_session_context", _impl)
...
def get_handoff_snapshot(session_id: str, tool_type: str) -> dict:
    ...
    return _run_tool_with_logging("get_handoff_snapshot", _impl)
...
def sync_session_memory(session_id: str, tool_type: str) -> dict:
    return _run_tool_with_logging(
        "sync_session_memory",
        lambda: local_memory.sync_session(session_id, tool_type),
    )
...
def search_memory(query: str, limit: int = 10) -> list[dict]:
    return _run_tool_with_logging(
        "search_memory",
        lambda: local_memory.search_memory(query, limit),
    )
