...
Graphiti MCP Server 提供语义记忆和知识图谱能力，需要 Docker 部署。

```bash
# 1. 配置环境变量
# 在 graphiti 项目目录下创建 .env，填入：
# OPENAI_API_KEY=sk-...

# 2. 启动服务
docker compose up -d

# 3. 将 CSM 会话数据同步到 Graphiti
python3 csm.py graphiti-sync --all
```
...
- `CSM_TUI_NAME_INPUT=curses`: inline curses editor (supports left/right$HOME/delete), but IME
...
- **CSM**: Structured session metadata (which session, files changed, commands run)
- **Graphiti**: Semantic-level knowledge (task goals, decision rationale, context links)
- **ck**: Code-level understanding (semantic code search)
...
Graphiti MCP Server provides semantic memory and knowledge graph capabilities. Requires Docker.

```bash
# 1. Configure environment variables
# Create .env in the graphiti project directory:
# OPENAI_API_KEY=sk-...

# 2. Start the service
docker compose up -d

# 3. Sync CSM session data to Graphiti
python3 csm.py graphiti-sync --all
```
